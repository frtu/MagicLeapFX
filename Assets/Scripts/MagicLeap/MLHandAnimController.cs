...
namespace MagicLeap
{
    ...
    public class MLHandAnimController : MonoBehaviour
    {
        #region Private Variables
        private const float CONFIDENCE_THRESHOLD = 0.95f;

        [SerializeField, Tooltip("Game object to animate.")]
        ...
        void Update()
        {
            ...
            if (GetKeyPoseConfidence(MLHands.Left, MLHands.Right, _keyPoseToToggleON)) 
            {
                _statusText.text = string.Format("Hand Gestures {0} Detected !!",
                    _keyPoseToToggleON.ToString());

                Animator ator = _targetObject.GetComponentInChildren<Animator>();
                ator.SetInteger("AnimState", 1);
            }
            
            // If active, listen to Deactivate signal
            if (GetKeyPoseConfidence(MLHands.Left, MLHands.Right, _keyPoseToToggleOFF)) 
            {
                _statusText.text = string.Format("Hand Gestures {0} Detected !!",
                    _keyPoseToToggleOFF.ToString());

                Animator ator = _targetObject.GetComponentInChildren<Animator>();
                ator.SetInteger("AnimState", 2);
            }
        }
        #endregion
